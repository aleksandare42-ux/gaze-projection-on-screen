...
def train_epoch(model, loader, criterion, optimizer, device):
    additiona_logger.info('Starting training epoch...')
    model.train()
    total_loss = 0.0
    for batch in loader:
...
def parse_args():
    ...
    p.add_argument('--yolo_model',type=str, default='./application/pkgs/models/yolov11n-face.pt')
    return p.parse_args()
...
def main():
    ...
    yolo = YOLO('./application/pkgs/models/yolov11n-face.pt')  # please download a face model or use yolov11n.pt and filter by class
    ...
    transform = T.Compose([
        T.ColorJitter(
            brightness=0.3,  # изменение яркости на ±30%
            contrast=0.2,    # изменение контраста на ±20%
        ),
    ])

    # Создаем дополнительный датасет с измененным освещением (25% от основного)
    augmented_size = int(len(train_df) * 0.25)
    augmented_df = train_df.sample(n=augmented_size, replace=True).reset_index(drop=True)
    
    # Создаем основной и дополнительный датасеты
    train_ds_main = GazeDataset(train_df, mode=args.mode, transform=None, yolo_model=yolo, fa=fa)
    train_ds_aug = GazeDataset(augmented_df, mode=args.mode, transform=transform, yolo_model=yolo, fa=fa)
    
    # Объединяем датасеты
    train_ds = ConcatDataset([train_ds_main, train_ds_aug])
    ...
    for epoch in range(1, args.epochs+1):
        ...
        # save checkpoint
        ckpt_path = os.path.join(args.outdir, f'last.pth')
        torch.save({
            'epoch': epoch,
